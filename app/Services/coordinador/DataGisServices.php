    ...
    public function DataGis($id)
    {
        try {
            ...
            if ($urlConsulta->failed()) {
                // Registrar el estado HTTP y el cuerpo de la respuesta en caso de fallo
                Log::error("Error al consultar el servicio GIS. Estado HTTP: " . $urlConsulta->status());
                Log::error("Cuerpo de la respuesta (en caso de fallo): " . $urlConsulta->body());
                return [
